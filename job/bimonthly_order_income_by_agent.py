...
from vendor.models import Supplier, SupplierManager, SalesAgent
from util.jsonall import json_encode
from django.db import connection


def order_income_by_signoff_by_agent(supplier_id=None, from_date=None, to_date=None, agent_id=None):
    """
    ...
    if agent_id is not None:
        sql_per_signoff = """
            ...
            from basedata_order as a
             ...
             where signoff_date >= %(from_date)s and signoff_date < %(to_date)s and agent_id = %(agent_id)s
             """
        ...
        sql_per_signoff = """
            ...
            from basedata_order as a
             ...
             inner join (select x.order_id, sum(if(x.cost, x.cost * x.pcs, y.cost * x.pcs)) as cost
                ...
                on x.product_id = y.code
                group by x.order_id) as c on a.order_no = c.order_id
             where signoff_date >= %(from_date)s and signoff_date < %(to_date)s and agent_id is NULL
             """
        ...
        summary_sql = """
            ...
            from basedata_order as a
             inner join (select x.order_id, sum(if(x.cost, x.cost * x.pcs, y.cost * x.pcs)) as cost
                ...
                on x.product_id = y.code
                group by x.order_id) as c on a.order_no = c.order_id
             where signoff_date >= %(from_date)s and signoff_date < %(to_date)s and a.supplier_id=%(supplier_id)s
                and agent_id = %(agent_id)s
            """
    else:
        summary_sql = """
...
agents = SalesAgent.objects.filter(is_active=True, create_time__lt=to_date).values_list('id', 'name')
msg = []
err = []
cnt = 0
...
agents = list(agents)
agents.append((None, '布丁'))
for supplier_id, supplier_name in suppliers:
    for agent_id, agent_name in agents:
        supplier_id = supplier_id or '[TMS]'
        if TmsReport.objects.filter(
                report_type='order_income_by_signoff_by_agent',
                start_time=from_date,
                end_time=to_date,
                owner="SUP-%s-%s" % (supplier_id, agent_id)).exists():
            msg.append('%s在%s的报表已创建，忽略' % (supplier_name, agent_name))
            continue
        try:
            print "正在统计供应商%s在%s的报表..." % (supplier_name, agent_name)
            report = order_income_by_signoff_by_agent(supplier_id=supplier_id, from_date=from_date,
                                                      to_date=to_date, agent_id=agent_id)
            if report and 'data' in report:
                cnt += 1
                rp = TmsReport.objects.create(
                    report_type='order_income_by_signoff_by_agent',
                    version=REPORT_VERSION,
                    title="[%s]在[%s]的销售账单: %s 元" % (supplier_name, agent_name, report.get('title_sum')),
                    start_time=from_date,
                    end_time=to_date,
                    header=json_encode(report.get('header')),
                    data=json_encode(report.get('data')),
                    summary=json_encode(report.get('summary')),
                    owner="SUP-%s-%s" % (supplier_id, agent_id or 0),
                )
                title_sum = 0.0
            else:
                err.append('未获得供应商[%s]在[%s]的统计数据' % (supplier_name, agent_name))
            print "完成"
        except Exception, e:
            print "失败！"
            logger.exception(e)
