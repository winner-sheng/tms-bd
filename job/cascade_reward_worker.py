...
try:
    ...
    sql = """
        select a.order_no
        from basedata_order as a, profile_enduser as b
        where a.referrer_id = b.uid and b.referrer is not null
            and pay_date BETWEEN DATE_ADD(CURDATE()- DAY(CURDATE())+1, INTERVAL -1 MONTH)
                        AND DATE_ADD(CURDATE(), INTERVAL - DAY(CURDATE())+1 DAY)
            and a.order_state not in (0, 4, 5, 96, 98, 999)
            and not exists (
                select id from promote_rewardrecord as p
                where p.order_no = a.order_no and reward_type=1
            )
    ...
    #     from basedata_order as a, profile_enduser as b
    #     where a.referrer_id = b.uid and b.referrer is not null
    #         and pay_date BETWEEN DATE_ADD(CURDATE(), INTERVAL - DAY(CURDATE())+1 DAY)
    ...
    print u'共有%s笔订单的小伙伴激励收益未记录' % len(order_nos)
    if len(order_nos) > 0:
    ...
    sql = """
            ...
            AND EXISTS(
                ...
                signoff_date BETWEEN DATE_ADD(CURDATE()- DAY(CURDATE())+1, INTERVAL -1 MONTH)
                            AND DATE_ADD(CURDATE(), INTERVAL - DAY(CURDATE())+1 DAY)
    ...
    date_range = (datetime.date(cur_year, cur_month, 1),
        ...
        sql = """
            SELECT bo2.order_no # , a.uid, a.referrer, bo.cnt
                ...
                join (
                    SELECT referrer_id, COUNT(order_no) AS cnt
                    FROM basedata_order AS b
                    where pay_date BETWEEN DATE_ADD(CURDATE()- DAY(CURDATE())+1, INTERVAL -1 MONTH)
                                AND DATE_ADD(CURDATE(), INTERVAL - DAY(CURDATE())+1 DAY)
                    and b.order_state not in (0, 4, 5, 96, 98, 999)
                    GROUP BY b.referrer_id
                    having cnt < %s
                ...
                join (
                    ...
                    WHERE signoff_date BETWEEN DATE_ADD(CURDATE()- DAY(CURDATE())+1, INTERVAL -1 MONTH)
                                AND DATE_ADD(CURDATE(), INTERVAL - DAY(CURDATE())+1 DAY)
                    and c.order_state not in (0, 4, 5, 96, 98, 999)
