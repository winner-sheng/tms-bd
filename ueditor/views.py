...
from tms.settings import QINIU_ACCESS_KEY, QINIU_SECRET_KEY, QINIU_BUCKET_NAME, QINIU_URL
...
def get_files(root_path, cur_path, allow_types=None):
    ...
    for item in items:
        ...
        if os.path.isdir(item_fullname):
            ...
            if is_allow_list:
                rel_path = os.path.relpath(cur_path, USettings.gSettings.MEDIA_ROOT)
                files.append({
                    "url": urllib.basejoin(USettings.gSettings.MEDIA_URL,
                                           os.path.join(rel_path, item).replace("\\", "/")),
...
def UploadFile(request):
    ...
    if action == "uploadscrawl":
        ...
        upload_file_name = file.name.lower()
    ...
    return_info = {
        ...
        'url': urllib.basejoin(QINIU_URL, OutputPathFormat),
...
def catcher_remote_image(request):
    ...
    for remote_url in remote_urls:
        #取得上传的文件的原始名称
        remote_file_name = os.path.basename(remote_url).lower()
...
def get_output_path(request, path_format, path_format_var):
    #取得输出文件的路径
    output_path_format = USettings.UEditorUploadSettings.get(path_format)
    if output_path_format:
        output_path = timezone.now().strftime(output_path_format)
    else:
        output_path = USettings.UEditorSettings["defaultPathFormat"] % path_format_var
    OutputPathFormat = output_path.replace("\\", "/")
