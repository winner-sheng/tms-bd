...
class Zz253Sms(object):
    ...
    def send_sms(text, mobile):
        ...
        能用接口发短信
            ...
            120	测试内容不是白名单
        """
        # a = SIGNATURE_FXJ if agent_id == 12 else SIGNATURE
        params = urllib.urlencode({'account': Zz253Sms.account,
                                   'pswd': Zz253Sms.password,
                                   'msg': "%s%s" % (SIGNATURE, text),
                                   'mobile': mobile,
                                   'needstatus': True,
                                   'extno': ''})
        headers = {"Content-type": "application/x-www-form-urlencoded", "Accept": "text/plain"}
...
def send_sms(mobile, template, data=None):
    ...
    for m in mobiles:
        if not is_valid_mobile(m):
            ...
            continue
        try:
            print SMS_MESSENGER()(m, template, data)
        except Exception, e:
            logger.exception(e)
...
def sms(mobile, msg):
    return sms.zz253.send_sms(msg, mobile)
