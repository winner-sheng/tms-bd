...
SPECIAL_CHARS_PATTERN = re.compile(u'[\~～\!！@#$%\^&\*\(\)_——\+\-=`\;；\'\./\{\}\|\:"><\?【】、。，￥]+')

def translate_special_char(s, delimiter=','):
    """
    将给定字符串中特殊字符删除，并把一个或多个连续的特殊字符转为delimiter指定的特殊字符
    :param s:
    :param chars:
    :param delimiter:
    :return:
    ...
    return SPECIAL_CHARS_PATTERN.sub(delimiter, s)
